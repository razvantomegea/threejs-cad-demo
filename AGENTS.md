...
- Use Claude Haiku 4.5 or GPT-5.5 for simple dumb tasks with single file context
- The plan must be very specific with small chunks, with code samples, with minimum 95% accuracy, and with explanations on each part for the reasoning. The plan must be able to be implemented in parallel by dumb AI agents which work on independent files. The plan must always be "In review" before implementing it and must be marked as "Ready for development" before executing.
